...
11. **Two-layer versioning — the anti-clash rule.** The vendored engine is a Marketplace **component**: a `catalog.json` item + a `ComponentId` with its own directory and `installed.json` (same pattern as Semantic Views / ExaPump), mirrored + released on its OWN tag line. Studio's **overlay** — MCP tool wiring, the DB-scoped profile, provider ranking, rebrand strings — lives in app-owned config in a DIFFERENT directory and is versioned with the app. A component update swaps only the engine payload under its dir; it never writes the overlay dir, so our edits and upstream bumps can never collide. The supervisor resolves "engine binary" from the component dir (falling back to the bundled baseline) and "config/overlay" from the app dir — two independent version lines, reported separately in About. Installers carry a baseline engine so first run is offline; the Updates tab moves it forward in place.
