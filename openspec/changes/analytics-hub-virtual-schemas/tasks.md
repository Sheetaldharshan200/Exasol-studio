...
- [ ] 2.5 Delete `virtual-schema-sources.ts` together with `NewVirtualSchema.tsx` in 4.4 — the old one-line entries and the new per-adapter contract differ enough that re-pointing the old form is throwaway work; verify `grep -rn virtual-schema-sources apps/desktop/src` is empty after 4.4
