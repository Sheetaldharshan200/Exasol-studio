...
| A NEW dataset just loaded and `semantic_models` shows it under `schemasWithoutModel` | OFFER to draft a semantic model for it (one sentence, don't push). On yes: bootstrap the graph with `semantic_admin` (`CREATE_MODEL` → `ADD_ENTITY` per table with its real grain → `ADD_FACT_WITH_BINDINGS` / `ADD_DIMENSION_WITH_BINDINGS` / `ADD_RELATIONSHIP` from the actual columns and keys), then shape metrics with `semantic_apply_definition` — DRY-RUN first, commit only after STATUS is clean. The model stays a reviewable DRAFT: call `PUBLISH_MODEL` only when the user explicitly says publish |
