...
export type InfoEditKind = "table" | "view" | "schema" | "user";

/** SQL keyword per object kind — RENAME <KIND> / COMMENT ON <KIND>. */
const KIND_SQL: Record<InfoEditKind, string> = {
  table: "TABLE",
  view: "VIEW",
  schema: "SCHEMA",
  user: "USER",
...
/**
 * Rename / comment (and for users: change password) from the Details → Info
 * tab — one editor for tables, views, schemas, and users. Same two-button
 * model as every other editor: Review SQL / Confirm & Save with the inline
 * error banner. Schemas and users are unqualified; tables/views keep their
 * schema on rename (Exasol renames within the same schema).
 */
export function ObjectInfoEditor({
  kind,
  schema,
  name,
  onOpenSql,
  ...
  const kw = KIND_SQL[kind];
  const unqualified = kind === "schema" || kind === "user";
  const fq = (n: string) => (unqualified ? `"${n}"` : qualify(schema, n));

  const statements = useMemo<string[]>(() => {
    const out: string[] = [];
    const to = newName.trim();
    if (to && to !== name) out.push(`RENAME ${kw} ${fq(name)} TO "${to}";`);
    // A rename runs first, so later statements target the new name.
    const finalName = to && to !== name ? to : name;
    if (comment.trim()) out.push(`COMMENT ON ${kw} ${fq(finalName)} IS '${comment.replace(/'/g, "''")}';`);
    // Exasol passwords are double-quoted identifiers.
    if (kind === "user" && password.trim()) out.push(`ALTER USER "${finalName}" IDENTIFIED BY "${password.replace(/"/g, '""')}";`);
    return out;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [newName, comment, password, name, schema, kind]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 flex-wrap items-center gap-2 border-b border-border px-2 py-1">
        ...
        <span className="font-mono text-[11px] text-muted-foreground">{fq(name)}</span>
        <button onClick={() => { setNewName(name); setComment(""); setPassword(""); setApplyError(null); }} disabled={!dirty} className="flex h-6 items-center gap-1 rounded-md border border-border px-1.5 text-[11px] text-muted-foreground hover:text-foreground disabled:opacity-40"><RotateCcw className="h-3.5 w-3.5" /> Revert</button>
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-auto p-3">
        <label className="block">
          <span className="mb-0.5 block text-[10px] font-medium uppercase tracking-wide text-muted-foreground">{kind === "user" ? "User name" : kind === "schema" ? "Schema name" : kind === "view" ? "View name" : "Table name"}</span>
          ...
          {!unqualified ? <span className="mt-0.5 block text-[10px] text-muted-foreground">Renames within schema {schema ?? "—"}.</span> : null}
        </label>
        <label className="block">
          <span className="mb-0.5 block text-[10px] font-medium uppercase tracking-wide text-muted-foreground">Comment</span>
          <textarea value={comment} onChange={(e) => setComment(e.target.value)} rows={3} placeholder={`Describe this ${kind}…`} className="w-full resize-y rounded-lg border border-border bg-transparent px-2.5 py-2 text-[12.5px] text-foreground outline-none placeholder:text-muted-foreground focus:border-primary/50" />
        </label>
        {kind === "user" ? (
          <label className="block">
            <span className="mb-0.5 block text-[10px] font-medium uppercase tracking-wide text-muted-foreground">New password</span>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Leave blank to keep the current password" className="h-8 w-full rounded-lg border border-border bg-transparent px-2.5 font-mono text-[12.5px] text-foreground outline-none focus:border-primary/50" />
          </label>
        ) : null}
