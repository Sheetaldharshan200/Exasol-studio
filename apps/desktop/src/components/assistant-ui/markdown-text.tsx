...
const CodeHeader: FC<CodeHeaderProps> = ({ language, code }) => {
  ...
  // Studio: SQL blocks get an "Apply to editor" action (drops the query into
  // the active workbench tab) when the panel provides a handler.
  ...
  return (
    <div className="aui-code-header-root border-border/50 bg-muted/50 mt-3 flex items-center justify-between rounded-t-xl border border-b-0 px-3.5 py-1.5 text-xs">
      ...
      {applySql && language?.toLowerCase() === "sql" ? (
        <TooltipIconButton tooltip="Apply to editor" onClick={onApply}>
          {!applied && <FileInputIcon className="animate-in zoom-in-75 fade-in duration-150" />}
          {applied && <CheckIcon className="text-primary animate-in zoom-in-50 fade-in duration-200 ease-out" />}
        </TooltipIconButton>
      ) : null}
