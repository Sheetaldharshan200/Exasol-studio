...
pub fn community_setup(app: AppHandle, action: String) -> AppResult<CommunityStatus> {
    match action.as_str() {
        "start-docker" => engine_start(&app, JOB_ID).map(|_| status_now()),
        "install-docker" => engine_install(&app, JOB_ID).map(|_| status_now()),
        other => Err(AppError::Storage(format!("Unsupported setup action: {other}"))),
    }
}

/// Start an INSTALLED engine and wait until it answers. `job_id` names the
/// market:log lane so any card (Community, AI Lab, …) can stream this.
pub(crate) fn engine_start(app: &AppHandle, job_id: &str) -> AppResult<String> {
    let Some(bin) = docker() else {
...
/// Install a headless engine (macOS: Homebrew + Colima) and bring it up.
pub(crate) fn engine_install(app: &AppHandle, job_id: &str) -> AppResult<String> {
    ...
    // Same patience as engine_start: the engine can take a moment to answer
    // after `colima start` returns — a fresh install must not read as failed.
    let started = std::time::Instant::now();
    while started.elapsed() < Duration::from_secs(90) {
    ...
    Err(AppError::Storage("Docker installed but the engine did not become ready within 90s — Re-check in a moment.".into()))
}

/// The zero-prerequisite path any install can lean on: use the running engine,
/// start a stopped one, or install one headlessly — never "install Docker
/// first" as a dead end.
pub(crate) fn ensure_engine_ready(app: &AppHandle, job_id: &str) -> AppResult<String> {
    match docker() {
        Some(bin) if docker_ok(&bin, &["info"]) => Ok(bin),
        Some(_) => engine_start(app, job_id),
        None => engine_install(app, job_id),
    }
}
